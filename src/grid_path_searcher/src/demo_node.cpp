...
{     
    if( wp.poses[0].pose.position.z < 0.0 || _has_map == false )
        return;
    ROS_INFO("start point is %f,%f,%f",_start_pt(0),_start_pt(1),_start_pt(2));
    Vector3d target_pt;
    target_pt << wp.poses[0].pose.position.x,
                 wp.poses[0].pose.position.y,
                 wp.poses[0].pose.position.z;
    ROS_INFO("target %f,%f,%f", wp.poses[0].pose.position.x, wp.poses[0].pose.position.y, wp.poses[0].pose.position.z);
...
{   
    ...
    // ROS_INFO("%d", cloud_vis.width );
    pcl::toROSMsg(cloud_vis, map_vis);
    ROS_INFO("map vis size %d", map_vis.width );
...
bool get_goal(grid_path_searcher::update_goal::Request &req,
              grid_path_searcher::update_goal::Response &res)
    {
        Vector3d target_pt;
        target_pt << req.x,req.y,0;
        ROS_INFO("target is %f,%f",target_pt(0),target_pt(1));
        res.result = "received";
        pathFinding(_start_pt, target_pt); 
        return true;
    }
...
int main(int argc, char** argv)
{
    ...
    service = nh.advertiseService("get_goal", get_goal);

    nh.param("map/cloud_margin",  _cloud_margin, 0.0);
    nh.param("map/resolution",    _resolution,   1.0);
    
    nh.param("map/x_size",        _x_size, 10.0);
    nh.param("map/y_size",        _y_size, 10.0);
    nh.param("map/z_size",        _z_size, 0.0 );
    
    nh.param("planning/start_x",  _start_pt(0),  0.0);
    nh.param("planning/start_y",  _start_pt(1),  0.0);
    nh.param("planning/start_z",  _start_pt(2),  0.0);

    _map_lower << 0.0, 0.0,   0.0;
    _map_upper << _x_size,  _y_size, _z_size;
